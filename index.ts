...
const roundRobinServices: AIService[] = [groqService, cerebrasService, openRouterService];
let rrIndex = 0;
...
const server = Bun.serve({
  ...
  async fetch(req) {
    ...
    if (req.method === 'GET' && pathname === '/services') {
      return new Response(
        JSON.stringify({ services: ['auto', ...Object.keys(serviceRegistry)] }),
    ...
    if (req.method === 'POST' && pathname === '/chat') {
      ...
      console.log(`[${new Date().toISOString()}] /chat → ${service.name} | msgs: ${messages.length} | service param: ${svcName ?? "auto"}`);
      ...
      const readable = new ReadableStream({
        async start(controller) {
          ...
          try {
            ...
            controller.enqueue(enc.encode(`data: ${JSON.stringify({ service: service.name })}\n\n`));
...
console.log(`📡 Services: ${Object.keys(serviceRegistry).join(', ')} + auto (round-robin)`);